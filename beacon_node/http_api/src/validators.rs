...
// Collects `f(validator_index, validator)` over the requested validators, or over all validators when
// `indices` is `None`.
...
// Similar with `collect_validators`, but also passes validator balance to `f`.
